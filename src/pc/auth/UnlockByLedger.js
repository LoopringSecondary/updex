...
import {Icon, Radio, Input, Spin, Steps, Modal as AntdModal} from 'antd';
...
class ChooseLedgerAddress extends React.Component {
  state = {
    connecting : false,
  ...
  render () {
    ...
    const handlePathChange = async (e) => {
      ...
      this.setState({connecting:true, showLedgerHelper:false})
      let ledger = null
      connectLedger().then(res => {
        if (!res.error) {
          ledger = res.result;
          return getXPubKey(dpath, ledger)
        } else {
        ...
        this.setState({connecting:false, showLedgerHelper:true})
      });
    };
            ...
            <Steps.Step status="process" title={''}
                        description={
                          <div>
                            ...
                            <Button onClick={() => hideLedgerHelper()} type="primary" className="mt5" loading={false}>{intl.get('unlock_by_ledger.close_helper')}</Button>
                          </div>
                        }
            />
          </Steps>
        </AntdModal>
        <Spin spinning={this.state.connecting}>
          <div className="">
