...
export default {
  ...
  effects:{
    ...
    *qrcodeGenerated({ payload={} }, { put }) {
      const {order, qrcode, hash, time} = payload
    ...
    * unsign({ payload={} }, { put }) {
      const {task, makerOrder, unsign} = payload
      yield put({ type: 'stepChange',payload:{step:1}});
      yield put({ type: 'taskChange',payload:{task}});
      yield put({ type: 'makerOrderChange',payload:{makerOrder}});
  ...
  reducers: {
    ...
    makerOrderChange(state, action) {
      const {payload} = action
      let {makerOrder} = payload
      return {
        ...state,
        makerOrder
      }
